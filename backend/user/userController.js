    ...
    User.findOne({ Email: email })
      .then((savedUser) => {
        ...
        savedUser
          ...
          .then((result) => {
            res.json({
              message: "Picture changed created successfully",
              data: savedUser,
            });
          })
...
exports.userData = (req, res, next) => {
  ...
  Promise.all(idsArray.map((idx) => User.findById(idx)))
    .then((users) => {
      users.forEach((user) => {
        if (user) {
          usersData.push(user);
      ...
      if (usersData.length > 0) {
        res.json({
          data: usersData.map((userData) => ({
            name: userData.Name,
            profilepic: userData.Photo ? new Buffer.from(userData.Photo, "base64") : null,
          })),
        });
      } else {
        res.json({ error: "No user was found for the provided IDs" });
      }
    })
    .catch((err) => next(err));

  // res.json({message: `SENT: ${req}`})
  // User.findById(id)
  // .then((user) => {
  // 	if(user) {
  // 		res.json({ data: user})
  // 	}
  // 	else {
  // 		return res.json({ error: "No other user was found" });
  // 	}
  // })
  // .catch(err =>
  // 	next(err))
